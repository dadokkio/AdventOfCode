...
def oddity(data, line, cell):
    counter = 0
    for index in range(cell):
        if data[index] in ["|", "J", "L"] and (line, index) in moves:
            counter += 1
    return counter % 2
...
for line in range(len(data) - 1):
    for cell in range(len(data[0]) - 1):
        if (line, cell) not in moves:
            odd = oddity(data[line], line, cell)
            total += odd
        else:
            odd = data[line][cell]
        print(odd, sep="", end="")
